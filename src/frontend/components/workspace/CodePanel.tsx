...
export default function CodePanel({ activeFilePath, activeFileContents }: CodePanelProps) {
  return (
    <section className="workspace-panel rounded-2xl border p-4 shadow-[0_8px_26px_rgb(34_32_28/6%)]">
      <h2 className="workspace-kicker">Code</h2>
      {activeFilePath ? (
        <div className="mt-2 space-y-2">
          <p className="text-sm font-medium text-[var(--foreground)]">{activeFilePath}</p>
          <pre className="overflow-auto rounded-xl bg-[var(--surface)] p-3 text-xs text-[var(--muted)]">
